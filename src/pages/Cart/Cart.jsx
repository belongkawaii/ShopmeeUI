...
function Cart() {
  ...
  async function handleUpdateQuantity(cartItemId, newQty, stockQty) {
    if (newQty < 1) return;
    if (newQty > stockQty) {
      showWarning(`Số lượng trong kho chỉ còn tối đa ${stockQty} cái.`);
      return;
    }
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        showError(result.message || "Cập nhật số lượng thất bại.");
      }
    } catch (err) {
      console.error(err);
  ...
  async function handleRemoveItem(cartItemId) {
    const confirmed = await showConfirm("Bạn có chắc chắn muốn xóa sản phẩm này khỏi giỏ hàng?");
    if (!confirmed) {
      return;
    }
  ...
  async function handleBulkRemove() {
    ...
    const confirmed = await showConfirm(`Bạn có chắc muốn xóa ${selectedItems.length} sản phẩm đã chọn?`);
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        showError(result.message || "Không thể xóa các sản phẩm đã chọn.");
      }
    } catch (err) {
      console.error(err);
      showError("Không thể thực hiện xóa hàng loạt.");
    }
  }
  ...
  if (placedOrder && (placedOrder.data?.payment_method !== "momo" || placedOrder.data?.payment_status === "paid")) {
    const orderId = placedOrder.data?.order_id;
    const total = placedOrder.data?.total_amount;
    const actualPaymentMethod = placedOrder.data?.payment_method || paymentMethod;
      ...
      <div className="cart-container success-page">
        <div className="success-card">
          ...
          <div className="order-details-box">
            ...
            <p><strong>Phương thức:</strong> {getPaymentMethodLabel(actualPaymentMethod)}</p>
          </div>
